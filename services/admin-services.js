  ...
  getUsers: (req, cb) => {
    ...
    Promise.all([
      User.findAll({
        attributes: {
          include: [
            [Sequelize.fn('COUNT', Sequelize.col('tweets.id')), 'tweetsCounts'],
            [Sequelize.fn('COUNT', Sequelize.col('Followers.id')), 'followersCounts'],
            [Sequelize.fn('COUNT', Sequelize.col('Followings.id')), 'followingsCounts']
          ]
        },
        include: [
          {
