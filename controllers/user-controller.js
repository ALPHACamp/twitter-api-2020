  ...
  putUserSetting: (req, res, next) => {
    ...
    return Promise.all([
      ...
      .then(([user, hash, checkAccount, checkEmail]) => {
        if (!user) throw new Error("User didn't exist!")
        if (checkAccount && checkAccount?.toJSON().account !== user.account) throw new Error('The account is existed!')
        if (checkEmail && checkEmail?.toJSON().email !== user.email) throw new Error('The email is existed!')
        return user.update({
          name,
          account,
          email,
