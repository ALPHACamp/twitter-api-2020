  ...
  getUserTweets: (req, res, next) => {
    return Promise.all([
      ...
      .then(([user, tweets]) => {
        ...
        const result = tweets.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
    ...
    return Promise.all([
      ...
      .then(([user, replies]) => {
        ...
        if (!replies || replies.length === 0) {
          return res.status(404).json({ status: 'error', message: 'No replies found' })
        }
    ...
    return Promise.all([
      ...
      .then(([user, likes]) => {
        ...
        if (!likes || likes.length === 0) {
          return res.status(404).json({ status: 'error', message: 'No likes found' })
        }
        // sort likes descending
        const result = likes.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
        return res.status(200).json(result)
      })
