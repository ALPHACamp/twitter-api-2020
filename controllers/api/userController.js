...
const userController = {
  ...
  getUser: async (req, res) => { //取得任一使用者資料
    ...
    return res.json({ id, name, account, email, avatar, cover, introduction, tweetsNumber, followingsNumber, followersNumber, isFollowed })
  },
  getUserTweets: async (req, res) => {
    const t = await Tweet.findAll({ where: { UserId: req.params.id }, order: [['createdAt', 'DESC']], include: [Reply, User, { model: User, as: 'LikedUsers' }] })
    const tweets = []
    t.map(async (tweet) => {
      let isLiked = ''
      if (Array.isArray(helpers.getUser(req).LikedTweets)) {
        isLiked = await helpers.getUser(req).LikedTweets.map(d => d.id).includes(id)// 是否按過like
      } else { isLiked === false }
  ...
  getUserReplies: async (req, res) => {
    ...
    reply.map(async (r) => {
      ...
      const likesNumber = LikedUsers.length // 推文like數
      const repliesNumber = Replies.length  // 推文回覆數
      const tweetData = { comment, id, description, likesNumber, repliesNumber, isLiked, createdAt, User: { id: User.id, name: User.name, account: User.account, avatar: User.avatar } }
  ...
  getUserFollowings: async (req, res) => { // 取得 :userId 的追蹤者
    ...
    const forTest = [{ followingId: Followings[0].Followship.followingId }]
    const followings = []
    Followings.map(async (d) => {
      let isFollowed = ''
      if (Array.isArray(helpers.getUser(req).Followings)) {
        isFollowed = await helpers.getUser(req).Followings.map(f => f.id).includes(d.id)
      } else { isFollowed === false }
      followings.push({
        id: d.id,
        name: d.name,
    ...
    return res.json(forTest, id, name, tweetsNumber, followings)
  },
  getUserFollowers: async (req, res) => {
    ...
    const forTest = [{ followerId: Followers[0].Followship.followerId }]
    const followers = []
    Followers.map(async (d) => {
      let isFollowed = ''
      if (Array.isArray(helpers.getUser(req).Followers)) {
        isFollowed = await helpers.getUser(req).Followers.map(f => f.id).includes(d.id)
      } else { isFollowed === false }
      followers.push({
        id: d.id,
        name: d.name,
    ...
    return res.json(forTest, id, name, tweetsNumber, followers)
  },
  getTopUsers: async (req, res) => {
    let users = await User.findAll({ include: [{ model: User, as: 'Followers' }] })
    users = users.map(async (user) => {
      const isFollowed = await helpers.getUser(req).Followings.map(d => d.id).includes(user.dataValues.id)
        ({
  ...
  getUserLikes: async (req, res) => {
    ...
    likes.map(async (like) => {
      ...
      let isLiked = ''
      if (Array.isArray(helpers.getUser(req).LikedTweets)) {
        isLiked = await helpers.getUser(req).LikedTweets.map(d => d.id).includes(id)
      } else { isLiked === false }
      tweets.push({
        TweetId: id,
        description,
