...
const authenticator = (req, res, next) => {
  return passport.authenticate('jwt', { session: false }, (err, user) => {
    ...
    if (user.role !== 'user') return res.status(403).json({ success: false, message: 'permission denied' }) // 如果角色錯誤則回傳403:使用者權限不足
...
const authenticatorAdmin = (req, res, next) => {
  return passport.authenticate('jwt', { session: false }, (err, user) => {
    ...
    if (user.role !== 'admin') return res.status(403).json({ success: false, message: 'permission denied' }) // 如果角色錯誤則回傳403:使用者權限不足
