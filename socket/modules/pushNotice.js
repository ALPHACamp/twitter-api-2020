  ...
  try {
    ...
    if (actions.includes(action)) {
      ...
      if (action === "tweet") {
        ...
        console.log(
          `NewNotices of userId:${currentUser.id}'s subscribers have updated.`
        )
        ...
        const targetUserOnline = usersOnline.find(
        ...
        if (targetUserOnline) {
          ...
          if (
            targetUserOnline.currentRoom &&
            targetUserOnline.currentRoom === "notice"
          ) {
          ...
          socket
            .to(targetUserOnline.socketId)
            .emit("server-push-notice", "new notice!")
          socket
            .to(targetUserOnline.socketId)
            .emit("server-update", usersOnline)
        }
      }
    } else {
